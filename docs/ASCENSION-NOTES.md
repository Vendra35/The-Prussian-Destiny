...
`in_game/common/scripted_relations/PD_alliance_addon.txt` is generated by
`tools/gen_alliance_addon.py`. While
`is_situation_active = situation:the_prussian_ascension`, an
**AI** Prussia can neither offer nor accept an alliance with the Emperor or with
anyone holding land in `north_german_region`, `prussia_area` or `silesia_area`.
That region is wider than its name: it also holds Holland, Friesland, Flanders,
Brabant and Wallonia (vanilla `map_data/definitions.txt:233-470`). The reason is
`disallow_war` on the alliance: an allied target cannot be attacked, so an AI
allied to one stalls the step. Since 2026-09-14 a human Prussia is not locked.

`pd_brandenburg.200` still cuts Prussia's alliances with owners in Silesia, both
Saxonies, Mecklenburg and Holstein, and with the Emperor, once at on_start and
for AI and human alike; a human may sign them again afterwards.
`PD_imperial_outcast_situation_2` carries no alliance effect of its own.

The lock tested `prussian_ascension_active` until 2026-09-14. `pd_brandenburg.299`
sets that flag without asking whether the situation can start, and `can_start`
refuses when Prussia already holds every target area
(`the_prussian_ascension.txt:7`), so the flag could stay set for good with an AI
Prussia locked. `is_situation_active` cannot; `AMBITION-NOTES.md` has the
history of all three keys.
