...
instead of throwing.
  Phase 1 -- while the_prussian_ambition is active, an AI Brandenburg may not
             ally anyone. Not only its targets: an alliance calls both ways
             (alliance.txt:15-16), and the situation picks wars and knocks
             out the Order only while Brandenburg is at peace
             (brandenburg_rise.txt:264-267, :453-455), so a distant ally's war
             freezes it as surely as an allied target.
...
WHY custom_tooltip IN offer_enabled. offer_enabled is drawn as the offer's
requirement list, and a bare is_ai there renders as "is_ai has no
localization" (seen in game 2026-09-14). Each lock is wrapped the way vanilla
wraps its own offer_enabled requirement (scripted_relations/
wako_sponsorship.txt:30-38), so the player reads one line instead of the
negated tree. should_ai_offer_trigger is the AI's own test and stays bare.
...
# The two tooltip keys live in main_menu/localization/english/PD_l_english.yml.
TT_AMBITION = "PD_alliance_lock_ambition_tt"
TT_ASCENSION = "PD_alliance_lock_ascension_tt"
...
def custom_tooltip(key, lines):
    return ["custom_tooltip = {", TAB + "text = %s" % key] + indent(lines, 1) + ["}"]
...
def phase2_lock(a, b):
    return [
        ...
        TAB * 2 + "OR = {",
    ] + indent(phase2_pair(a, b), 3) + indent(phase2_pair(b, a), 3) + [
        TAB * 2 + "}",
        TAB + "}",
        "}",
    ]
...
def locks(a, b, with_tooltips):
    p1, p2 = phase1_lock(a, b), phase2_lock(a, b)
    if with_tooltips:
        p1, p2 = custom_tooltip(TT_AMBITION, p1), custom_tooltip(TT_ASCENSION, p2)
    return [
        "",
...
def generate():
    ...
    text = insert_before_close(text, "should_ai_offer_trigger", locks("first", "second", False))
    text = insert_before_close(text, "offer_enabled", locks("actor", "recipient", True))
